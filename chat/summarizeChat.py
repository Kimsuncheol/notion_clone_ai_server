...
_MODEL_NAME = os.getenv("OPENAI_CHAT_MODEL", "gpt-4.1-mini")

chat = ChatOpenAI(model=_MODEL_NAME, api_key=os.getenv("OPENAI_API_KEY"), temperature=0.1)
...
def _maybe_seed_history(history: ChatMessageHistory, seed_history: Optional[List[str]]) -> None:
  ...
  history.add_message(SystemMessage(content=f"Earlier conversation summary:\n{joined_seed}"))
...
# def summary_chat_service(content: str, note_id: str, chat_history_seed: Optional[List[str]] = None) -> str:
# 변경
def summary_chat_service(content: str, note_id: str, chat_history_seed: Optional[List[object]] = None) -> str:
  if not note_id:
...
# 기존 함수 대체
def _maybe_seed_history(history: ChatMessageHistory, seed_history: Optional[List[object]]) -> None:
  if not seed_history or history.messages:
    return
